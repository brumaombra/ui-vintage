...
<template>
  <AlertDialog :open="Boolean(currentDialog)" @update:open="handleOpenChange">
    <AlertDialogContent v-if="currentDialog">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start">
          <div class="mx-auto flex size-12 shrink-0 items-center justify-center rounded border border-[var(--border-light)] bg-[var(--bg-surface-light)] sm:mx-0 sm:size-10 dark:border-[var(--border-dark)] dark:bg-[var(--bg-surface-dark)]">
            <component
              :is="confirmIcon"
              :stroke-width="1.8"
              class="size-6 text-[var(--button-primary-light)] dark:text-[var(--button-primary-dark)]"
            />
          </div>
      ...
      <AlertDialogFooter>
        <Button :variant="currentDialog.options.cancelButtonType" class="w-full sm:w-auto" :disabled="confirmDialogState.busy" @click="cancelActiveConfirmDialog">
          {{ currentDialog.options.cancelText }}
        </Button>
        <Button :variant="currentDialog.options.confirmButtonType" class="w-full sm:w-auto" :disabled="confirmDialogState.busy" @click="confirmActiveDialog">
